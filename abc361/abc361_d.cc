...
constexpr llong kBlack = 0;
constexpr llong kWhite = 1;
constexpr llong kEmpty = 2;

llong encode(std::string_view state) {
  llong encoded = 0;

  for (llong i = state.size() - 1; i >= 0; i--) {
    encoded <<= 2;
...
llong bitSwap(llong n, llong b1, llong b2) {
  if (((n >> b1) & 1) != ((n >> b2) & 1)) {
    n ^= 1ll << b1;
    n ^= 1ll << b2;
  }
...
llong search(const std::string &S, const std::string &T) {
  auto goal = encode(T + "..");
  std::queue<std::tuple<llong, llong, llong>> queue;
  std::unordered_set<llong> visited;
  queue.emplace(encode(S + ".."), llong(S.size()), 0);

  while (!queue.empty()) {
    auto [state, empBegin, dist] = queue.front();
    queue.pop();
    ...
    if (!visited.contains(state)) {
      ...
      for (llong i = 0; i <= S.size(); i++) {
        auto cell = (state >> (2 * i)) & 0b11;
        auto cellNext = (state >> (2 * (i + 1))) & 0b11;

        if (cell != kEmpty && cellNext != kEmpty) {
          auto newState = state;
          newState = bitSwap(newState, 2 * i, 2 * empBegin);
          newState = bitSwap(newState, 2 * i + 1, 2 * empBegin + 1);
          newState = bitSwap(newState, 2 * i + 2, 2 * empBegin + 2);
          newState = bitSwap(newState, 2 * i + 3, 2 * empBegin + 3);
          queue.emplace(newState, i, dist + 1);
        }
      }
    }
  }
