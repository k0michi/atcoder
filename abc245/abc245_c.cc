...
void answer() {
  ...
  std::vector<std::vector<bool>> table(N, std::vector<bool>(2));
  table[0][0] = table[0][1] = true;

  for (llong i = 1; i < N; i++) {
    table[i][0] = table[i - 1][0] && std::abs(A[i - 1] - A[i]) <= K ||
                  table[i - 1][1] && std::abs(B[i - 1] - A[i]) <= K;
    table[i][1] = table[i - 1][0] && std::abs(A[i - 1] - B[i]) <= K ||
                  table[i - 1][1] && std::abs(B[i - 1] - B[i]) <= K;
  }

  writeln(table[N - 1][0] || table[N - 1][1] ? "Yes" : "No");
}
