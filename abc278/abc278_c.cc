...
void answer() {
  llong N, Q;
  read(N, Q);
  std::unordered_map<llong, std::unordered_set<llong>> follows;
  ...
  for (llong i = 0; i < Q; i++) {
    ...
    if (T_i == 1) {
      follows[A_i].insert(B_i);
    } else if (T_i == 2) {
      follows[A_i].erase(B_i);
    } else {
      bool ff = follows[A_i].contains(B_i) && follows[B_i].contains(A_i);
      writeln(ff ? "Yes" : "No");
    }
  }
}
