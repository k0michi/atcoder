...
void answer() {
  llong N, H, W;
  read(N, H, W);
  std::vector<std::pair<llong, llong>> AB(N);
  read(AB);

  std::vector<llong> order(N);
  std::iota(order.begin(), order.end(), 0);
  bool can = false;

  do {
    for (llong i = 0; i < (1 << N); i++) {
      std::vector<llong> grid(W);

      for (llong k = 0; k < N; k++) {
        std::pair<llong, llong> topLeft{H, 0};

        for (llong l = 0; l < W; l++) {
          topLeft = std::min(topLeft, {grid[l], l});
        }

        auto [width, height] = AB[order[k]];

        if ((i >> k) & 1) {
          std::swap(width, height);
        }

        if (topLeft.second + width > W) {
          goto fail;
        }

        for (llong x = 0; x + 1 < width; x++) {
          if (grid[topLeft.second + x] != grid[topLeft.second + x + 1]) {
            goto fail;
          }
        }

        for (llong x = 0; x < width; x++) {
          grid[topLeft.second + x] += height;
        }
