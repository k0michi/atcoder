...
AdjList prim(llong nodeCount, const AdjMatrix &matrix) {
  AdjList result(nodeCount);
  ...
  for (llong i = 0; i < nodeCount; ++i) {
    ...
    if (parents[u] != -1) {
      result[parents[u]].push_back({u, minCost[u]});
      result[u].push_back({parents[u], minCost[u]});
    }
...
std::vector<llong> distancesFrom(llong start, llong nodeCount,
                                 const AdjList &list) {
  std::vector<llong> distances(nodeCount, -1);
...
void answer() {
  llong N;
  read(N);
  std::vector<std::vector<llong>> A(N, std::vector<llong>(N));
  ...
  for (llong i = 0; i < N; i++) {
    auto dists = distancesFrom(i, N, mst);
