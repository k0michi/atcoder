...
void answer() {
  llong N, K;
  read(N, K);
  std::vector<std::vector<llong>> ans;

  std::vector<llong> init(N);
  init[0] = K;
  std::queue<std::pair<llong, std::vector<llong>>> q;
  q.push({2, init});

  while (!q.empty()) {
    auto seq = q.front();
    ans.push_back(seq.second);
    q.pop();

    for (llong i = seq.first; i <= N; i++) {
      if (seq.second[0] - i >= 0) {
        auto seq2 = seq;
        seq2.first = i;
        seq2.second[0] -= i;
