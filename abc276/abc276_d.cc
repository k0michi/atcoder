...
void answer() {
  ...
  std::unordered_map<llong, llong> factorsMin;

  for (llong i = 0; i < N; i++) {
    auto factors = primeFactorize(a[i]);
    factors[2], factors[3];

    for (auto &&[f, c] : factors) {
      factorsMin.try_emplace(f, std::numeric_limits<llong>::max());
      factorsMin[f] = std::min(factorsMin[f], c);
    }
  }
