...
std::vector<std::set<llong>> columns, rows;

void erase(llong c, llong r) {
  columns[c].erase(r);
  rows[r].erase(c);
}
...
void answer() {
  ...
  for (llong i = 0; i < Q; i++) {
    ...
    if (columns[C_i].contains(R_i) && rows[R_i].contains(C_i)) {
      erase(C_i, R_i);
      continue;
    }

    auto it = columns[C_i].lower_bound(R_i);

    if (it != columns[C_i].end()) {
      erase(C_i, *it);
    }

    it = columns[C_i].lower_bound(R_i);

    if (it != columns[C_i].begin()) {
      erase(C_i, *(--it));
    }

    it = rows[R_i].lower_bound(C_i);
