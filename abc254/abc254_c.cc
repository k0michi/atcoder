...
void answer() {
  ...
  std::vector<std::vector<llong>> subseqs(K);

  for (llong i = 0; i < N; i++) {
    subseqs[i % K].push_back(a[i]);
  }

  for (llong i = 0; i < K; i++) {
    std::sort(subseqs[i].begin(), subseqs[i].end());
  }

  bool sorted = true;

  for (llong i = 1; i < N; i++) {
    if (subseqs[(i - 1) % K][(i - 1) / K] > subseqs[i % K][i / K]) {
      sorted = false;
    }
  }

  writeln(sorted ? "Yes" : "No");
}
