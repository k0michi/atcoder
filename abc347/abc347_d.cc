...
void answer() {
  llong a, b, C;
  read(a, b, C);
  llong cCount = std::popcount(ullong(C));
  std::pair<llong, llong> counts = {-1, -1};

  for (llong i = 0; i <= cCount; i++) {
    if (a - i == b - (cCount - i) && a - i >= 0 && a - i <= 60 - cCount) {
      counts = {i, cCount - i};
    }
  }
