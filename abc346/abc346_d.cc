...
void answer() {
  ...
  std::vector<llong> cumu0(N + 1), cumu1(N + 1);

  for (llong i = 1; i <= N; i++) {
    cumu0[i] = cumu0[i - 1] + ((i - 1) % 2 == toInt(S[i - 1]) ? 0 : C[i - 1]);
    cumu1[i] =
        cumu1[i - 1] + ((i - 1 + 1) % 2 == toInt(S[i - 1]) ? 0 : C[i - 1]);
  }

  llong min = std::numeric_limits<llong>::max();

  for (llong i = 0; i + 1 < N; i++) {
    llong cost0 = 0, cost1 = 0;
    cost0 += toInt(S[i]) == 0 ? 0 : C[i];
    cost0 += toInt(S[i + 1]) == 0 ? 0 : C[i + 1];

    if (i % 2 == 0) {
      cost0 += cumu0[i] - cumu0[0];
      cost0 += cumu1[N] - cumu1[i + 2];
    } else {
      cost0 += cumu1[i] - cumu1[0];
      cost0 += cumu0[N] - cumu0[i + 2];
    }

    cost1 += toInt(S[i]) == 1 ? 0 : C[i];
    cost1 += toInt(S[i + 1]) == 1 ? 0 : C[i + 1];

    if (i % 2 == 0) {
      cost1 += cumu1[i] - cumu1[0];
      cost1 += cumu0[N] - cumu0[i + 2];
    } else {
      cost1 += cumu0[i] - cumu0[0];
      cost1 += cumu1[N] - cumu1[i + 2];
    }

    min = std::min({min, cost0, cost1});
  }
