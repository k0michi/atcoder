...
std::tuple<llong, llong, llong> extendedGCD(llong a, llong b) {
  if (b == 0) {
    return {a, 1, 0};
  } else {
...
template <ullong M> struct ModInt {
  static constexpr llong Modulus = M;

  constexpr ModInt() : mValue(0) {}

  template <typename T, std::enable_if_t<std::is_signed_v<T>> * = nullptr>
  constexpr ModInt(T value) {
    llong llValue = llong(value);
    llValue %= Modulus;
  ...
  constexpr ModInt(T value) : mValue(value % Modulus) {}

  constexpr llong toLLong() const { return llong(mValue); }

  constexpr ullong toULLong() const { return mValue; }
  ...
  constexpr ModInt<M> &operator/=(const ModInt<M> &r) {
    return *this = *this * invert(r);
  }

  constexpr ModInt<M> &operator+() const { return *this; }

  constexpr ModInt<M> &operator-() const { return ModInt<M>{} - *this; }

  constexpr ModInt<M> &operator++() { return *this += 1; }
  ...
  constexpr bool operator==(const ModInt<M> &r) const {
    return mValue == r.mValue;
  }

  constexpr bool operator!=(const ModInt<M> &r) const { return !(*this == r); }

  static constexpr ModInt<M> pow(const ModInt<M> &a, ullong exp) {
    if (exp == 0) {
      return 1;
    }

    auto result = pow(a, exp / 2);
    result *= result;
  ...
  static constexpr ModInt<M> invert(const ModInt<M> &a) {
    auto inverse = extendedGCD(llong(a.mValue), llong(M));
    return {std::get<1>(inverse)};
  }

  friend constexpr ModInt<M> operator+(const ModInt<M> &l, const ModInt<M> &r) {
    return ModInt<M>(l) += r;
  }
  ...
  friend constexpr std::ostream &operator<<(std::ostream &s,
                                            const ModInt<M> &a) {
    return s << a.mValue;
  }

  friend constexpr std::istream &operator>>(std::istream &s, ModInt<M> &a) {
    return s >> a.mValue;
  }
...
llong floorLog(llong x, llong base) {
  llong l = 0;
...
ModInt998244353 positioning(llong count, llong width) {
  if (count == 1) {
    return 1;
  }
  ...
  auto result = positioning(half, width);
  result += result * ModInt998244353::pow(10, half * width);

  if (count % 2 != 0) {
    result *= ModInt998244353::pow(10, width);
    result += 1;
  }
...
void answer() {
  llong N;
  read(N);
  writeln(positioning(N, floorLog(N, 10) + 1) * N);
}
