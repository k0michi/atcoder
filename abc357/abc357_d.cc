...
llong intWidth(llong x, llong base) {
  return x == 0 ? 0 : floorLog(x, base) + 1;
}

void answer() {
  llong N;
  read(N);
  auto ceil = ModInt998244353::pow(10, intWidth(N, 10));
  auto result = (ModInt998244353::pow(ceil, N) - 1) / (ceil - 1);
  result *= N;
  writeln(result);
}
