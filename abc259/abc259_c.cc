...
std::vector<std::pair<char, llong>> runLengthEncode(std::string_view s) {
  char last;
  llong length = 0;
  std::vector<std::pair<char, llong>> result;
...
void answer() {
  std::string S, T;
  read(S, T);
  auto sEncoded = runLengthEncode(S);
  auto tEncoded = runLengthEncode(T);

  bool can = false;

  if (sEncoded.size() != tEncoded.size()) {
    goto out;
  } else {
    for (llong i = 0; i < sEncoded.size(); i++) {
      if (sEncoded[i].first != tEncoded[i].first) {
        goto out;
      } else {
        if (sEncoded[i].second > tEncoded[i].second ||
            sEncoded[i].second < tEncoded[i].second &&
                std::min(sEncoded[i].second, tEncoded[i].second) == 1) {
          goto out;
        }
      }
    }
