...
ullong pow2(ullong x) { return x * x; }

void answer() {
  llong T;
  read(T);
  for (llong i = 0; i < T; i++) {
    llong X_1, Y_1, R_1, X_2, Y_2, R_2;
    read(X_1, Y_1, R_1, X_2, Y_2, R_2);
    ullong d = pow2(std::abs(X_1 - X_2)) + pow2(std::abs(Y_1 - Y_2));
    writeln(pow2(std::abs(R_1 - R_2)) <= d && d <= pow2(R_1 + R_2) ? "Yes"
                                                                   : "No");
  }
}
