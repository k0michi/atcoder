...
void answer() {
  llong N;
  read(N);
  std::vector<llong> K(N);
  std::vector<std::vector<llong>> A(N);
  ...
  std::vector<std::unordered_map<llong, double>> dices(N);
  double max = 0;

  for (llong i = 0; i < N; i++) {
    std::unordered_map<llong, llong> counts;

    for (llong j = 0; j < K[i]; j++) {
      counts[A[i][j]]++;
    }

    for (auto &&[n, c] : counts) {
      dices[i][n] = double(c) / K[i];
    }
  ...
  writeln(std::fixed, std::setprecision(9), max);
}
