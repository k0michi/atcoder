...
void answer() {
  ...
  H += M / 60;
  auto hX = std::cos(H / 6 * std::numbers::pi) * A;
  auto hY = std::sin(H / 6 * std::numbers::pi) * A;
  auto mX = std::cos(M / 30 * std::numbers::pi) * B;
  auto mY = std::sin(M / 30 * std::numbers::pi) * B;
  writeln(std::fixed, std::setprecision(10), euclideanDistance(hX, hY, mX, mY));
}
