...
void answer() {
  llong N, X, Y;
  read(N, X, Y);
  std::vector<llong> redJewels(N + 1), blueJewels(N + 1);
  redJewels[N] = 1;

  for (llong i = N; i >= 2; i--) {
    redJewels[i - 1] += redJewels[i];
    blueJewels[i] += X * redJewels[i];
    redJewels[i - 1] += blueJewels[i];
    blueJewels[i - 1] += Y * blueJewels[i];
  }

  writeln(blueJewels[1]);
}
