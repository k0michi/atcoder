...
bool isPalindromic(std::string_view s) {
  std::string reversed{s};
  std::reverse(reversed.begin(), reversed.end());
  return s == reversed;
}

constexpr llong kBase = 998244353;
...
void answer() {
  ...
  std::vector<std::unordered_map<std::string, llong>> table(2);
  table[0][""] = 1;
  ...
  for (llong i = 1; i <= N; i++) {
    ...
    for (auto [suffix, comb] : table[1]) {
      for (auto c : "AB"sv) {
        if (S[i - 1] == '?' || S[i - 1] == c) {
          auto newSuffix = suffix + c;

          if (newSuffix.size() == K && isPalindromic(newSuffix)) {
            continue;
          }

          if (newSuffix.size() == K) {
            newSuffix = newSuffix.substr(1);
          }

          table[0][newSuffix] += comb;
          table[0][newSuffix] %= kBase;
        }
      }
    }
  }
