...
void answer() {
  std::unordered_map<llong, std::unordered_set<llong>> access;
  std::unordered_set<llong> globalAccess;
  ...
  for (llong i = 0; i < Q; i++) {
    ...
    if (op == 1) {
      ...
      globalAccess.insert(X);
    } else {
      read(Y);
      llong can = globalAccess.contains(X) || access[Y].contains(X);
      writeln(can ? "Yes" : "No");
    }
  }
}
