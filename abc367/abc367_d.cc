...
void answer() {
  ...
  std::vector<llong> distances(2 * N);

  for (llong i = 1; i < 2 * N; i++) {
    distances[i] = (distances[i - 1] + A[(i - 1) % N]) % M;
  }

  std::vector<llong> counts(M);
  ...
  for (llong i = N; i < 2 * N; i++) {
    counts[distances[i - N]]--;
    count += counts[distances[i]];
    counts[distances[i]]++;
  }
