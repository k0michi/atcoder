...
std::pair<T, T> binarySearch(T left, T right, F f) {
  while (right - left > 1) {
...
llong integralSqrt(llong n) {
  return binarySearch(0ll, n + 1, [&](llong x) { return x > 0 && x > n / x; })
      .first;
}
...
void answer() {
  ...
  llong count = 0;
  count += integralSqrt(N / 2);
  count += integralSqrt(N / 4);
  writeln(count);
}
