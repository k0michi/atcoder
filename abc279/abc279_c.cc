...
void answer() {
  llong H, W;
  read(H, W);
  std::vector<std::string> columnsS(W, std::string(H, ' ')),
      columnsT(W, std::string(H, ' '));
  ...
  std::sort(columnsS.begin(), columnsS.end());
  std::sort(columnsT.begin(), columnsT.end());
  writeln(columnsS == columnsT ? "Yes" : "No");
}
