...
void answer() {
  llong N, Q;
  read(N, Q);
  std::vector<llong> balls(N), indices(N + 1);
  ...
  for (llong i = 0; i < Q; i++) {
    llong x_i;
    read(x_i);
    llong index = indices[x_i];
    llong indexRight = indices[x_i] + 1;

    if (indexRight == N) {
      indexRight = N - 2;
    }

    std::swap(indices[x_i], indices[balls[indexRight]]);
    std::swap(balls[index], balls[indexRight]);
  }
