...
void answer() {
  llong N;
  read(N);
  std::vector<llong> X(N), P(N);
  read(X, P);

  std::vector<llong> cumulative(N + 1);

  for (llong i = 1; i <= N; i++) {
    cumulative[i] = cumulative[i - 1] + P[i - 1];
  }
  ...
  for (llong i = 0; i < Q; i++) {
    ...
    read(L_i, R_i);
    auto l =
        std::distance(X.begin(), std::lower_bound(X.begin(), X.end(), L_i));
    auto u =
        std::distance(X.begin(), std::upper_bound(X.begin(), X.end(), R_i));
    writeln(cumulative[u] - cumulative[l]);
  }
}
