...
void answer() {
  ...
  std::vector<std::vector<bool>> table(N + 1, std::vector<bool>(X + 1));
  table[0][0] = true;
  ...
  for (llong i = 1; i <= N; i++) {
    ...
    for (llong j = 0; j <= X; j++) {
      if (j - a_i >= 0) {
        table[i][j] = table[i][j] || table[i - 1][j - a_i];
      }

      if (j - b_i >= 0) {
        table[i][j] = table[i][j] || table[i - 1][j - b_i];
      }
    }
  }

  writeln(table[N][X] ? "Yes" : "No");
}
