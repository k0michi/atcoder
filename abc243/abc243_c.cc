...
void answer() {
  llong N;
  read(N);
  std::vector<std::pair<llong, llong>> XY(N);
  read(XY);

  std::unordered_map<llong, std::set<std::pair<llong, char>>> rows;

  for (llong i = 0; i < N; i++) {
    char S_i;
    read(S_i);
    rows[XY[i].second].insert({XY[i].first, S_i});
  }

  bool collide = false;

  for (auto &&[y, row] : rows) {
    auto lastD = row.begin()->second;
