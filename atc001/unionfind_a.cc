...
class DisjointSet {
  std::vector<llong> mParents;
  std::vector<llong> mRanks;

public:
  explicit DisjointSet(llong elementCount)
      : mParents(elementCount), mRanks(elementCount) {
    for (llong i = 0; i < elementCount; i++) {
      mParents[i] = i;
    }
  }
  ...
  llong root(llong x) {
    if (mParents[x] == x) {
      ...
      return mParents[x] = root(mParents[x]);
    }
  }

  bool compareRoot(llong x, llong y) { return root(x) == root(y); }

  llong unite(llong x, llong y) {
    auto xRoot = root(x);
    auto yRoot = root(y);
    ...
    if (mRanks[xRoot] < mRanks[yRoot]) {
      std::swap(xRoot, yRoot);
    }

    mParents[yRoot] = xRoot;
  ...
  llong elementCount() { return llong(mParents.size()); }
};
...
void answer() {
  ...
  for (llong i = 0; i < Q; i++) {
    ...
    if (P_i == 0) {
      ...
      writeln(set.compareRoot(A_i, B_i) ? "Yes" : "No");
    }
  }
}
