...
void answer() {
  ...
  std::vector<llong> products;

  for (auto &&[k, v] : counts) {
    products.push_back(k * v);
  }

  std::sort(products.begin(), products.end());

  for (llong i = 0; i < std::min<llong>(K, products.size()); i++) {
    products[products.size() - i - 1] = 0;
  }

  llong min = 0;

  for (llong i = 0; i < products.size(); i++) {
    min += products[i];
  }
