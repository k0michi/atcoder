...
void answer() {
  ...
  std::vector<llong> cumuA(N + 1);
  std::vector<llong> cumuB(N + 1);
  std::vector<llong> cumuC(N + 1);

  for (llong i = 0; i < N; i++) {
    cumuA[i + 1] = cumuA[i] + (S[i] == 'A');
    cumuB[i + 1] = cumuB[i] + (S[i] == 'B');
    cumuC[i + 1] = cumuC[i] + (S[i] == 'C');
  }

  std::unordered_map<llong, llong> countAB;
  std::unordered_map<llong, llong> countAC;
  std::unordered_map<llong, llong> countBC;
  std::unordered_map<std::pair<llong, llong>, llong> countABAC;

  for (llong i = 0; i <= N; i++) {
    countAB[cumuA[i] - cumuB[i]]++;
    countAC[cumuA[i] - cumuC[i]]++;
    countBC[cumuB[i] - cumuC[i]]++;
    countABAC[{cumuA[i] - cumuB[i], cumuA[i] - cumuC[i]}]++;
  }

  llong result = (N + 1) * N / 2;

  for (auto &&[k, v] : countAB) {
    result -= v * (v - 1) / 2;
  }

  for (auto &&[k, v] : countAC) {
    result -= v * (v - 1) / 2;
  }

  for (auto &&[k, v] : countBC) {
    result -= v * (v - 1) / 2;
  }

  for (auto &&[k, v] : countABAC) {
    result += v * (v - 1);
  }
