...
void answer() {
  ...
  llong centerIndex = ones.size() / 2;
  llong center = ones[centerIndex];
  llong swaps = 0;

  for (llong i = 0; i < ones.size(); i++) {
    llong dest = center - centerIndex + i;
    swaps += std::abs(dest - ones[i]);
  }
