...
void answer() {
  llong N, Q;
  read(N, Q);
  std::vector<llong> indexToName(N);
  std::iota(indexToName.begin(), indexToName.end(), 0);
  std::vector<llong> nameToIndex(N);
  std::iota(nameToIndex.begin(), nameToIndex.end(), 0);
  std::vector<llong> nestIndices(N);
  std::iota(nestIndices.begin(), nestIndices.end(), 0);
  ...
  for (llong i = 0; i < Q; i++) {
    ...
    if (op == 1) {
      read(b), b--;
      llong nestIndex = nameToIndex[b];
      nestIndices[a] = nestIndex;
    } else if (op == 2) {
      read(b), b--;
      std::swap(indexToName[nameToIndex[a]], indexToName[nameToIndex[b]]);
      std::swap(nameToIndex[a], nameToIndex[b]);
    } else {
      writeln(indexToName[nestIndices[a]] + 1);
    }
  }
}
