...
void answer() {
  ...
  read(A);
  std::vector<std::vector<llong>> table(2, std::vector<llong>(N));
  table[0][0] = 0;
  table[1][0] = A[0];

  for (llong i = 1; i < N; i++) {
    table[0][i] = std::max(table[0][i - 1], table[1][i - 1] + 2 * A[i]);
    table[1][i] = std::max(table[1][i - 1], table[0][i - 1] + A[i]);
  }

  writeln(std::max(table[0][N - 1], table[1][N - 1]));
}
