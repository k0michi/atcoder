...
bool within(llong x, llong end) { return x >= 0 && x < end; }
...
void answer() {
  ...
  for (llong y = 0; y < 9; y++) {
    for (llong x = 0; x < 9; x++) {
      for (llong y2 = y; y2 < 9; y2++) {
        for (llong x2 = x + 1; x2 < 9; x2++) {
          llong yd = y2 - y;
          llong xd = x2 - x;

          if (S[y][x] == '#' && S[y2][x2] == '#' && within(x - yd, 9) &&
              within(y + xd, 9) && within(x2 - yd, 9) && within(y2 + xd, 9) &&
              S[y + xd][x - yd] == '#' && S[y2 + xd][x2 - yd] == '#') {
            count++;
          }
        }
      }
