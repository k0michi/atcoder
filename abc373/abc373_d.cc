...
llong N, M;
std::vector<std::vector<std::pair<llong, llong>>> edges;
constexpr llong kNull = std::numeric_limits<llong>::min();
std::vector<llong> vertices;
...
void dfs(llong v) {
  ...
  for (auto &&e : edges[v]) {
    if (vertices[e.first] == kNull) {
      vertices[e.first] = x_v - e.second;
      dfs(e.first);
    }
  }
}
...
void answer() {
  ...
  for (llong i = 0; i < M; i++) {
    ...
    u_i--;
    v_i--;
    edges[u_i].push_back({v_i, -w_i});
    edges[v_i].push_back({u_i, w_i});
  }
