...
llong count(llong x, llong y, std::unordered_set<llong> numbers) {
  if (x >= W || y >= H) {
    return 0;
  }

  numbers.insert(A[y][x]);

  if (x == W - 1 && y == H - 1) {
    return numbers.size() == (W + H - 1);
  }

  llong result = 0;
  result += count(x + 1, y, numbers);
  result += count(x, y + 1, numbers);
  return result;
}

void answer() {
  read(H, W);
  A.resize(H, std::vector<llong>(W));
  read(A);
  writeln(count(0, 0, {}));
}
