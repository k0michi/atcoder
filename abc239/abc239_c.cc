...
void answer() {
  llong x_1, y_1, x_2, y_2;
  read(x_1, y_1, x_2, y_2);
  std::set<std::pair<llong, llong>> candidates;

  for (llong x = x_1 - 2; x <= x_1 + 2; x++) {
    for (llong y = y_1 - 2; y <= y_1 + 2; y++) {
      if (manhattanDistance(x, y, x_1, y_1) == 3) {
        candidates.insert({x, y});
      }
    }
  }

  for (llong x = x_2 - 2; x <= x_2 + 2; x++) {
    for (llong y = y_2 - 2; y <= y_2 + 2; y++) {
      if (manhattanDistance(x, y, x_2, y_2) == 3) {
